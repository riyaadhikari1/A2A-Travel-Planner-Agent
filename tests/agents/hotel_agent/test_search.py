import pytest
from unittest.mock import AsyncMock, MagicMock, patch

from a2a.types import Message, Part, Role, TaskState

from agents.hotel_agent.search import HotelExecutor


def make_context(instruction="Find hotels in Bangkok"):
    msg            = Message()
    msg.role       = Role.Value("ROLE_USER")
    msg.message_id = "test-message-id"
    part           = Part()
    part.text      = instruction
    msg.parts.append(part)

    ctx                             = MagicMock()
    ctx.task_id                     = "t-hotel"
    ctx.context_id                  = "ctx-hotel"
    ctx.message                     = msg
    ctx.current_task                = None
    ctx.get_user_input.return_value = instruction
    return ctx


def make_event_queue():
    q = MagicMock()
    q.enqueue_event = AsyncMock()
    return q


MOCK_SERPER_RESPONSE = {
    "organic": [
        {
            "title":    "Mandarin Oriental Bangkok",
            "snippet":  "Luxury hotel on the Chao Phraya River",
            "link":     "https://www.mandarinoriental.com/bangkok",
            "position": 1,
        },
        {
            "title":    "Chatrium Hotel Bangkok",
            "snippet":  "Contemporary riverside hotel",
            "link":     "https://www.chatrium.com",
            "position": 2,
        },
    ],
    "places": [
        {
            "title":       "Novotel Bangkok",
            "address":     "Siam Square, Bangkok",
            "rating":      4.2,
            "ratingCount": 1200,
            "link":        "https://novotel.com",
        }
    ],
}


def make_mock_http_client(response=None):
    resp                  = MagicMock()
    resp.json             = MagicMock(return_value=response or MOCK_SERPER_RESPONSE)
    resp.raise_for_status = MagicMock()

    client            = AsyncMock()
    client.post       = AsyncMock(return_value=resp)
    client.__aenter__ = AsyncMock(return_value=client)
    client.__aexit__  = AsyncMock(return_value=False)
    return client


@pytest.mark.asyncio
class TestHotelExecutorSuccess:

    async def test_enqueues_working_then_completed(self):
        executor    = HotelExecutor()
        context     = make_context()
        event_queue = make_event_queue()

        with patch("agents.hotel_agent.search.httpx.AsyncClient",
                   return_value=make_mock_http_client()):
            with patch("agents.hotel_agent.search.settings") as mock_settings:
                mock_settings.serper_api_key = "test-key"
                await executor.execute(context, event_queue)

        states = [
            call[0][0].status.state
            for call in event_queue.enqueue_event.call_args_list
            if hasattr(call[0][0], "status")
        ]
        assert TaskState.TASK_STATE_WORKING   in states
        assert TaskState.TASK_STATE_COMPLETED in states

    async def test_enqueues_artifact(self):
        executor    = HotelExecutor()
        context     = make_context()
        event_queue = make_event_queue()

        with patch("agents.hotel_agent.search.httpx.AsyncClient",
                   return_value=make_mock_http_client()):
            with patch("agents.hotel_agent.search.settings") as mock_settings:
                mock_settings.serper_api_key = "test-key"
                await executor.execute(context, event_queue)

        artifact_events = [
            call[0][0]
            for call in event_queue.enqueue_event.call_args_list
            if hasattr(call[0][0], "artifact")
        ]
        assert len(artifact_events) == 1

    async def test_posts_to_serper_api(self):
        executor    = HotelExecutor()
        context     = make_context()
        event_queue = make_event_queue()
        mock_client = make_mock_http_client()

        with patch("agents.hotel_agent.search.httpx.AsyncClient",
                   return_value=mock_client):
            with patch("agents.hotel_agent.search.settings") as mock_settings:
                mock_settings.serper_api_key = "test-key"
                await executor.execute(context, event_queue)

        mock_client.post.assert_called_once()

    async def test_parses_city_from_instruction(self):
        executor    = HotelExecutor()
        context     = make_context("Find hotels in Tokyo")
        event_queue = make_event_queue()
        mock_client = make_mock_http_client()

        with patch("agents.hotel_agent.search.httpx.AsyncClient",
                   return_value=mock_client):
            with patch("agents.hotel_agent.search.settings") as mock_settings:
                mock_settings.serper_api_key = "test-key"
                await executor.execute(context, event_queue)

        call_json = mock_client.post.call_args[1]["json"]
        assert "Tokyo" in call_json["q"]

    async def test_returns_results_from_organic_and_places(self):
        executor    = HotelExecutor()
        context     = make_context()
        event_queue = make_event_queue()

        with patch("agents.hotel_agent.search.httpx.AsyncClient",
                   return_value=make_mock_http_client()):
            with patch("agents.hotel_agent.search.settings") as mock_settings:
                mock_settings.serper_api_key = "test-key"
                await executor.execute(context, event_queue)

        artifact_events = [
            call[0][0]
            for call in event_queue.enqueue_event.call_args_list
            if hasattr(call[0][0], "artifact")
        ]
        assert len(artifact_events) == 1


@pytest.mark.asyncio
class TestHotelExecutorFailure:

    async def test_missing_serper_key_enqueues_failed(self):
        executor    = HotelExecutor()
        context     = make_context()
        event_queue = make_event_queue()

        with patch("agents.hotel_agent.search.settings") as mock_settings:
            mock_settings.serper_api_key = ""
            await executor.execute(context, event_queue)

        states = [
            call[0][0].status.state
            for call in event_queue.enqueue_event.call_args_list
            if hasattr(call[0][0], "status")
        ]
        assert TaskState.TASK_STATE_FAILED in states

    async def test_no_city_in_instruction_enqueues_failed(self):
        executor    = HotelExecutor()
        context     = make_context("show me some hotels")
        event_queue = make_event_queue()

        with patch("agents.hotel_agent.search.settings") as mock_settings:
            mock_settings.serper_api_key = "test-key"
            await executor.execute(context, event_queue)

        states = [
            call[0][0].status.state
            for call in event_queue.enqueue_event.call_args_list
            if hasattr(call[0][0], "status")
        ]
        assert TaskState.TASK_STATE_FAILED in states

    async def test_http_error_enqueues_failed(self):
        executor    = HotelExecutor()
        context     = make_context()
        event_queue = make_event_queue()

        client            = AsyncMock()
        client.post       = AsyncMock(side_effect=Exception("timeout"))
        client.__aenter__ = AsyncMock(return_value=client)
        client.__aexit__  = AsyncMock(return_value=False)

        with patch("agents.hotel_agent.search.httpx.AsyncClient",
                   return_value=client):
            with patch("agents.hotel_agent.search.settings") as mock_settings:
                mock_settings.serper_api_key = "test-key"
                await executor.execute(context, event_queue)

        states = [
            call[0][0].status.state
            for call in event_queue.enqueue_event.call_args_list
            if hasattr(call[0][0], "status")
        ]
        assert TaskState.TASK_STATE_FAILED in states

    async def test_never_raises_to_caller(self):
        executor    = HotelExecutor()
        context     = make_context()
        event_queue = make_event_queue()

        client            = AsyncMock()
        client.post       = AsyncMock(side_effect=RuntimeError("unexpected"))
        client.__aenter__ = AsyncMock(return_value=client)
        client.__aexit__  = AsyncMock(return_value=False)

        with patch("agents.hotel_agent.search.httpx.AsyncClient",
                   return_value=client):
            with patch("agents.hotel_agent.search.settings") as mock_settings:
                mock_settings.serper_api_key = "test-key"
                await executor.execute(context, event_queue)